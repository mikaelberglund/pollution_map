...
# xin = df.x[0][np.where(np.logical_and(df.x[0] >= np.quantile(a=df.x[0],q=0.025),df.x[0] <= np.quantile(a=df.x[0],q=0.975)))]
# yin = df.y[0][np.where(np.logical_and(df.y[0] >= np.quantile(a=df.y[0],q=0.025),df.y[0] <= np.quantile(a=df.y[0],q=0.975)))]
# xin = np.sum(np.logical_and(df.x[0] >= np.quantile(a=df.x[0],q=0.025),df.x[0] <= np.quantile(a=df.x[0],q=0.975)),axis=1) == df.x[0].shape[1]
xin = np.logical_and(df.x >= np.quantile(a=df.x,q=0.025),df.x <= np.quantile(a=df.x,q=0.975))
yin = np.logical_and(df.y >= np.quantile(a=df.y,q=0.025),df.y <= np.quantile(a=df.y,q=0.975))
f = np.array(xin&yin)
x_train = x_train[f]
y_train = y_train[f]
### NORMALIZE
