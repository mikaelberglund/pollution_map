...
locations = re.get('https://api.openaq.org/v1/locations?country[]=SE')
...
def get_last(imagecol,s,e,ar):
    ...
    while ~found_last:
        ...
        # im_test = imagecol
        #im_test = ee.Image(im_test.median()).rename(['result'])
        im_test = ee.Image(im_test.first())
        if (im_test.getInfo() is not None) & testds:
            ...
            im_test = im_test.reduceRegion(reducer=ee.Reducer.toList(), geometry=ar, maxPixels=1e13, scale=200,tileScale=8) #TODO: Sätt maxPixels så att alla får samma storlek?
...
x_train = np.empty(shape=(1, 12, 23))
y_train = np.empty(shape=(1))
for i in dftot.id.unique():
    print(i) #Todo: Varför har jag dubbla värden för vissa id:n?
    im = dftot[dftot.id == i][['latitude', 'longitude', 'tropospheric_NO2_column_number_density']]\
        .drop_duplicates().pivot('latitude','longitude','tropospheric_NO2_column_number_density').values
    if np.shape(im)[1]<23:
