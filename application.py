...
def getdf(temparea):
    ...
    result = result.reduceRegion(reducer=ee.Reducer.toList(), geometry=temparea, maxPixels=262144,
                            bestEffort=True, crs='EPSG:4326', scale=200)
    sample = result.sampleRectangle(region=temparea)
    band = sample.get('tropospheric_NO2_column_number_density')

    im = np.array(band.getInfo())
